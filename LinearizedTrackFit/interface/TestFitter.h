      ...
      std::unordered_map<int, std::pair<float, float> > & radiusCuts, bool singleModules, bool phiSymmetricFit)
  {
    ...
    while (treeReader.nextTrack()) {
      ...
      if (inputVarNames[0] == "CorrectedPhiSecondOrder") {
        ...
        double chargeOverPt = chargeOverPtEstimator.chargeOverPt(treeReader.getVarX(), treeReader.getVarY(),
                                                                 treeReader.layersFound());
        // std::cout << "pt = " << pt << std::endl;

        if (fabs(1 / chargeOverPt) <= 10.) {
          double normChi2 = linearFitLowPt.normChi2(varsVec, lastLadder);
          std::vector<float> principalComponents(linearFitLowPt.principalComponents(varsVec, lastLadder));
          std::vector<float> normalizedPrincipalComponents(linearFitLowPt.normalizedPrincipalComponents(varsVec, lastLadder));
          std::vector<float> estimatedPars(linearFitLowPt.trackParameters(varsVec, lastLadder));

          linearFitterHistograms.fill(vars, principalComponents, normalizedPrincipalComponents, pars, estimatedPars,
                                      normChi2);
          summaryHistograms.fill(vars, principalComponents, normalizedPrincipalComponents, pars, estimatedPars,
                                 normChi2,
          ...
          std::vector<float> principalComponents(linearFitHighPt.principalComponents(varsVec, lastLadder));
          std::vector<float> normalizedPrincipalComponents(linearFitHighPt.normalizedPrincipalComponents(varsVec, lastLadder));
          std::vector<float> estimatedPars(linearFitHighPt.trackParameters(varsVec, lastLadder));
        ...
        std::vector<float> principalComponents(linearFitLongitudinal.principalComponents(varsVec, lastLadder));
        std::vector<float> normalizedPrincipalComponents(linearFitLongitudinal.normalizedPrincipalComponents(varsVec, lastLadder));
        std::vector<float> estimatedPars(linearFitLongitudinal.trackParameters(varsVec, lastLadder));
